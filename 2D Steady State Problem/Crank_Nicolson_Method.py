...
def solve_3rd_Order_UDS(m, Dx, m_t, Dt, d, c) :
    ...
    higher_order_coeffs_np1, higher_order_coeffs_n = Coefficients.Coefficients_3rd_Order_Crank_Nicolson_Method(d, c)
    lower_order_coeffs_np1, lower_order_coeffs_n   = Coefficients.Coefficients_1st_Order_Crank_Nicolson_Method(d, c)
    ...
    for i, arr in enumerate([H, G, F, E, D]) :
        ...
        arr[1:-1]   = higher_order_coeffs_np1[i]
        arr[0]      = lower_order_coeffs_np1[i]
        arr[-1]     = lower_order_coeffs_np1[i]
    ...
    def f(phi) :
        ...
        p1 = Coefficients.multiply(phi[ :3], lower_order_coeffs_n)
        p3 = Coefficients.multiply(phi[-3:], lower_order_coeffs_n)

        p2 = Coefficients.multiply( np.column_stack( ( phi[4:], phi[3:-1], phi[2:-2], phi[1:-3], phi[:-4] ) ), higher_order_coeffs_n)
...
def solve_2nd_Order_CDS(m, Dx, m_t, Dt, d, c) :
    ...
    coeffs_np1, coeffs_n  = Coefficients.Coefficients_2nd_Order_Crank_Nicolson_Method(d, c)
    ...
    def f(phi) :
        ...
        return Coefficients.multiply( np.column_stack( ( phi[2:], phi[1:-1], phi[:-2] ) ), coeffs_n )

    # Solving for phi
    # while marching forward in time
    for n in range(1, m_t + 1) :

        b[:] = phi[n-1, 1:-1] + f(phi[n])
...
def solve_1st_Order_UDS(m, Dx, m_t, Dt, d, c) :
    ...
    coeffs_np1, coeffs_n  = Coefficients.Coefficients_1st_Order_Crank_Nicolson_Method(d, c)
